...
const validateOptionsFns =
  superSchemaOptionsFns as unknown as ValidatorFunctions;
...
describe("AJV8PrecompiledValidator", () => {
  ...
  describe("default options", () => {
    ...
    describe("validator.isValid()", () => {
      it("should return true if the data is valid against the schema", () => {
        ...
        expect(validator.isValid(schema, { name: "bar" }, rootSchema)).toBe(
          true,
        );
      });
      it("should return false if the data is not valid against the schema", () => {
        ...
        expect(validator.isValid(schema, { name: 12345 }, rootSchema)).toBe(
          false,
        );
      });
      it("should return false if junk option id is passed", () => {
        ...
        expect(validator.isValid(schema, { name: "foo" }, rootSchema)).toBe(
          false,
        );
      });
      it("should throw if the schema is not recognized", () => {
        ...
        expect(() =>
          validator.isValid(schema, { name: "bar" }, rootSchema),
        ).toThrow(
          new Error(
            `No precompiled validator function was found for the given schema for "${hash}"`,
          ),
        );
      });
      it("should throw if the rootSchema is different than the one the validator was constructed with", () => {
        ...
        expect(() =>
          validator.isValid(schema, { foo: { name: "bar" } }, schema),
        ).toThrow(
          new Error(
            "The schema associated with the precompiled validator differs from the rootSchema provided for validation",
          ),
    ...
    describe("validator.validateFormData()", () => {
      ...
      describe("Validating required fields", () => {
        ...
        describe("formData is not provided at top level", () => {
          ...
          it("should return an errorSchema", () => {
            ...
            expect(errorSchema.passwords!.pass2!.__errors![0]).toEqual(
              "must have required property 'pass2'",
            );
          });
        });
      ...
      describe("No custom validate function, single additionalProperties value of wrong type", () => {
        ...
        it("should return an errorSchema", () => {
          ...
          expect(errorSchema.anything!.foo!.__errors![0]).toEqual(
            "must be string",
          );
        });
      });
      ...
      describe("Custom validate function", () => {
        ...
        describe("formData is provided, but fails custom validation", () => {
          ...
          it("should return an error list", () => {
            expect(errors).toHaveLength(1);
            expect(errors[0].stack).toEqual(
              ".passwords.pass2 passwords don`t match.",
            );
          });
          it("should return an errorSchema", () => {
            expect(errorSchema.passwords!.pass2!.__errors).toHaveLength(1);
        ...
        describe("formData is missing data", () => {
          ...
          it("should return an errorSchema", () => {
            ...
            expect(errorSchema.passwords!.pass2!.__errors![1]).toEqual(
              "passwords don`t match.",
            );
          });
        });
      ...
      describe("Data-Url validation", () => {
        ...
        it("Data-Url with bad data generates error", () => {
          ...
          expect(result.errorSchema.dataUrlWithName!.__errors![0]).toEqual(
            'must match format "data-url"',
          );
        });
      });
    });
  ...
  describe("validator.validateFormData(), custom options, and localizer", () => {
    ...
    beforeAll(() => {
      ...
      validator = new AJV8PrecompiledValidator(
        validateOptionsFns,
        rootSchema,
        localizer,
      );
