(function (global, factory) {
    ...
    const errorList = errors.map((e) => {
      ...
      if (rawPropertyNames.length > 0) {
        rawPropertyNames.forEach((currentProperty) => {
          ...
          let uiSchemaTitle = utils.getUiOptions(
  ...
  function processRawValidationErrors(validator, rawErrors, formData, schema, customValidate, transformErrors, uiSchema) {
    ...
    const newFormData = utils.getDefaultFormState(
      validator,
      schema,
      formData,
      schema,
    ...
    const errorHandler = customValidate(
      newFormData,
      utils.createErrorHandler(newFormData),
      uiSchema,
      errorSchema
    );
  ...
  var AJV8PrecompiledValidator = class {
    ...
    ensureSameRootSchema(schema, formData) {
      if (!utils.deepEquals(schema, this.rootSchema)) {
        const resolvedRootSchema = utils.retrieveSchema(
          this,
          this.rootSchema,
          this.rootSchema,
          formData
