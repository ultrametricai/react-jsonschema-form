  ...
  const errorList = errors.map((e) => {
    ...
    if (rawPropertyNames.length > 0) {
      rawPropertyNames.forEach((currentProperty) => {
        ...
        let uiSchemaTitle = (0, import_utils2.getUiOptions)(
          (0, import_get.default)(uiSchema, `${path.replace(/^\./, "")}`)
        ).title;
      ...
      const uiSchemaTitle = (0, import_utils2.getUiOptions)(
        (0, import_get.default)(uiSchema, `${property.replace(/^\./, "")}`)
      ).title;
      if (uiSchemaTitle) {
...
function processRawValidationErrors(validator, rawErrors, formData, schema, customValidate, transformErrors, uiSchema) {
  ...
  const newFormData = (0, import_utils2.getDefaultFormState)(
    validator,
    schema,
    formData,
    schema,
  ...
  const errorHandler = customValidate(
    newFormData,
    (0, import_utils2.createErrorHandler)(newFormData),
    uiSchema,
    errorSchema
  );
...
var AJV8PrecompiledValidator = class {
  ...
  ensureSameRootSchema(schema, formData) {
    if (!(0, import_utils4.deepEquals)(schema, this.rootSchema)) {
      const resolvedRootSchema = (0, import_utils4.retrieveSchema)(
        this,
        this.rootSchema,
        this.rootSchema,
        formData
