...
import {
  ...
  sampleUISchema,
  SIMPLE_ONEOF,
  SIMPLE_ONEOF_OPTIONS,
} from "./testData/layoutData";
...
// eslint-disable-next-line no-unused-vars
const LOOKUP_MAP: {
  [index: string]: string | ((props: FieldProps) => ReactElement);
} = {
  FooClass: "Foo",
  BarClass: "Bar",
  TestRenderer,
...
const TEST_LAYOUT_GRID_CHILDREN = {
  [GridType.ROW]: [
    { [GridType.COLUMN]: ["column1"] },
    { [GridType.COLUMN]: ["column2"] },
  ],
  [GridType.COLUMN]: {
    className: "FooClass",
    children: ["column1"],
...
const FIELD_PATH_ID = toFieldPathId(
  DEFAULT_ID,
  readonlySchemaRegistry.globalFormOptions,
);
...
function getExpectedPropsForField(
  ...
  const schema = paths.reduce((result, name) => {
    const schema1 = schemaUtils.retrieveSchema(
      get(result, [PROPERTIES_KEY, name]) as RJSFSchema,
      props.schema,
    );
  ...
  const fieldPathId = fieldPathIdFromPaths(
    paths,
    globalFormOptions,
    props.fieldPathId!,
  );
...
describe("LayoutGridField", () => {
  function getProps(
    overrideProps: Partial<LayoutGridFieldProps> = {},
  ): LayoutGridFieldProps {
    const {
      formData,
      schema = {},
  ...
  describe("conditionMatches()", () => {
    ...
    test("returns true for ALL operator and values === data, arrays", () => {
      expect(conditionMatches(Operators.ALL, ["6", "7"], ["7", "6"])).toBe(
        true,
      );
    });
    ...
    test("returns true for SOME operator and values ∩ data, array", () => {
      expect(conditionMatches(Operators.SOME, ["6", "7"], ["6", "8"])).toBe(
        true,
      );
    });
    ...
    test("returns false for NONE operator and values ∩ data, array", () => {
      expect(conditionMatches(Operators.NONE, ["6", "7"], ["6", "8"])).toBe(
        false,
      );
    });
  ...
  describe("findChildrenAndProps()", () => {
    ...
    test("returns the children array and empty grid props for the row", () => {
      expect(
        findChildrenAndProps(TEST_LAYOUT_GRID_CHILDREN, GridType.ROW, registry),
      ).toEqual({
    ...
    test("returns the children array and looked up className in grid props for the column", () => {
      const expectedResult = {
        ...
        gridProps: {
          className:
            LOOKUP_MAP[TEST_LAYOUT_GRID_CHILDREN[GridType.COLUMN].className],
        },
      };
      expect(
        findChildrenAndProps(
          TEST_LAYOUT_GRID_CHILDREN,
          GridType.COLUMN,
          registry,
        ),
    ...
    test("returns the children array and expected looked up className values in grid props for the condition", () => {
      const classNames: string[] =
        TEST_LAYOUT_GRID_CHILDREN[GridType.CONDITION].className.split(" ");
      ...
      const expectedResult = {
        ...
        gridProps: {
          ...omit(TEST_LAYOUT_GRID_CHILDREN[GridType.CONDITION], ["children"]),
          className,
        },
      };
      expect(
  ...
  describe("computeArraySchemasIfPresent()", () => {
    test("returns undefined rawSchema and given fieldPathId for non-numeric potentialIndex", () => {
      expect(
        computeArraySchemasIfPresent(undefined, FIELD_PATH_ID, "string"),
      ).toEqual({
        rawSchema: undefined,
        fieldPathId: FIELD_PATH_ID,
      });
    ...
    test("returns undefined rawSchema and given fieldPathId for numeric potentialIndex, no schema", () => {
      expect(
        computeArraySchemasIfPresent(undefined, FIELD_PATH_ID, "0"),
      ).toEqual({
        rawSchema: undefined,
        fieldPathId: FIELD_PATH_ID,
      });
    ...
    test("returns undefined rawSchema and given fieldPathId for numeric potentialIndex, non-array schema", () => {
      expect(
        computeArraySchemasIfPresent(readonlySchema, FIELD_PATH_ID, "0"),
      ).toEqual({
        rawSchema: undefined,
        fieldPathId: FIELD_PATH_ID,
      });
    ...
    test("returns outer array rawSchema and generated fieldPathId for numeric potentialIndex, array schema", () => {
      const startPathId = toFieldPathId(
        "0",
        arraySchemaRegistry.globalFormOptions,
        FIELD_PATH_ID,
      );
      const fieldPathId = {
      ...
      expect(
        computeArraySchemasIfPresent(outerArraySchema, startPathId, "0"),
      ).toEqual({
  ...
  describe("getSchemaDetailsForField(), blank schema", () => {
    beforeAll(() => {
      ...
      findSelectedOptionInXxxOf = jest.spyOn(
    ...
    test("returns no schema or options when name is empty string", () => {
      expect(
        getSchemaDetailsForField(registry, "", {}, {}, FIELD_PATH_ID),
      ).toEqual(NO_SCHEMA_OR_OPTIONS);
      expect(retrieveSchemaSpy).toHaveBeenCalledTimes(1);
    });
    test("returns no schema or options when schema is empty", () => {
      expect(
        getSchemaDetailsForField(registry, "name", {}, {}, FIELD_PATH_ID),
      ).toEqual(NO_SCHEMA_OR_OPTIONS);
      expect(retrieveSchemaSpy).toHaveBeenCalledTimes(1);
    });
  });
  describe("getSchemaDetailsForField(), sampleSchema", () => {
    ...
    test("returns no schema or options when schema is missing the leaf field", () => {
      ...
      const fieldPathId = fieldPathIdFromPaths(
        paths,
        sampleSchemaRegistry.globalFormOptions,
        FIELD_PATH_ID,
      );
      expect(
        getSchemaDetailsForField(
  ...
  describe("getSchemaDetailsForField(), simpleOneOfSchema", () => {
    ...
    test("returns schema, isRequired: false, isReadonly: true, options: undefined when oneOf schema is used, passed idSeparator", () => {
      ...
      expect(
        ...
        fieldPathId: toFieldPathId(
          path,
          simpleOneOfRegistry.globalFormOptions,
          FIELD_PATH_ID,
        ),
      });
  ...
  describe("getSchemaDetailsForField(), gridFormSchema", () => {
    ...
    test("returns schema, isRequired: true, isReadonly: undefined, options when oneOf schema is requested", () => {
      ...
      const { field: schema } =
        gridFormSchemaRegistry.schemaUtils.findFieldInSchema(
          GRID_FORM_SCHEMA,
          path,
        );
      retrieveSchemaSpy.mockClear();
      expect(
        ...
        fieldPathId: toFieldPathId(
          path,
          gridFormSchemaRegistry.globalFormOptions,
          FIELD_PATH_ID,
        ),
        optionsInfo: {
          options: get(schema, [ONE_OF_KEY]),
    ...
    test("returns schema, isRequired: true, isReadonly: undefined, options: undefined when drilling through oneOf schema to prop", () => {
      ...
      const schema: RJSFSchema =
        gridFormSchemaRegistry.schemaUtils.getFromSchema(
          GRID_FORM_SCHEMA,
      ...
      expect(
        ...
        isReadonly: undefined,
        optionsInfo: undefined,
        fieldPathId: fieldPathIdFromPaths(
          paths,
          gridFormSchemaRegistry.globalFormOptions,
          FIELD_PATH_ID,
        ),
  ...
  describe("getSchemaDetailsForField(), readonlySchema", () => {
    ...
    test("returns schema, isRequired: false, isReadonly: undefined, options when oneOf schema is requested", () => {
      ...
      const { field: schema } =
        readonlySchemaRegistry.schemaUtils.findFieldInSchema(
          readonlySchema,
          path,
        );
      retrieveSchemaSpy.mockClear();
    ...
    test("returns schema, isRequired: true, isReadonly: true, options: undefined when selecting field on readonly parent", () => {
      ...
      const paths = path.split(".");
      const schema = get(readonlySchema, [
        PROPERTIES_KEY,
        "nested",
        PROPERTIES_KEY,
        "roNumber",
  ...
  describe("getCustomRenderComponent()", () => {
    ...
    test("returns the render function when render is a string with a lookup", () => {
      expect(getCustomRenderComponent("TestRenderer", registry)).toBe(
        TestRenderer,
      );
    });
  ...
  describe("computeFieldUiSchema()", () => {
    ...
    test("field with empty uiProps, uiSchema having readonly false in ui:options for the field and schemaReadonly unspecified and forceReadonly true", () => {
      ...
      expect(
        computeFieldUiSchema("foo", {}, uiSchema, undefined, true),
      ).toEqual({
        fieldUiSchema: {
          ...uiSchema.foo,
  ...
  describe("computeUIComponentPropsFromGridSchema()", () => {
    ...
    test("gridSchema contains name and looked up placeholder", () => {
      ...
      expect(
        computeUIComponentPropsFromGridSchema(registry, gridSchema),
      ).toEqual({
        name: "foo",
        uiProps: {
  ...
  test("renders nothing when there is no uiSchema", () => {
    ...
    const uiComponent = screen.queryByTestId(
      LayoutGridField.TEST_IDS.uiComponent,
    );
    const field = screen.queryByTestId(LayoutGridField.TEST_IDS.field);
    const layoutMultiSchemaField = screen.queryByTestId(
  ...
  test("renderField with render=TestRenderer via LAYOUT_GRID_OPTION", () => {
    ...
    const uiComponent = screen.getByTestId(
      LayoutGridField.TEST_IDS.uiComponent,
    );
  ...
  test("renderField via name explicit layoutGridSchema", async () => {
    ...
    const fieldPathId = toFieldPathId(
      fieldName,
      props.registry.globalFormOptions,
      props.fieldPathId,
    );
    ...
    expect(field).toHaveTextContent(
      stringifyProps(getExpectedPropsForField(props, fieldName)),
    );
  ...
  test("renderField via object explicit layoutGridSchema, otherProps", () => {
    ...
    expect(field).toHaveTextContent(
      stringifyProps(
        getExpectedPropsForField(props, fieldName, otherProps, otherUIProps),
      ),
    );
  });
  ...
  test("renderRow not nested", () => {
    ...
    expect(fields[0]).toHaveTextContent(
      stringifyProps(getExpectedPropsForField(props, GRID_CHILDREN[0])),
    );
    // Second field as the second child
    expect(fields[1]).toHaveTextContent(
      stringifyProps(getExpectedPropsForField(props, GRID_CHILDREN[1])),
    );
  });
