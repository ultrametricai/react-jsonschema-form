...
describeRepeated("Form common", (createFormComponent) => {
  describe("Empty schema", () => {
    it("Should throw error when Form is missing validator", () => {
      expect(() =>
        createComponent(Form, {
          ref: createRef(),
          schema: {},
          validator: undefined as unknown as ValidatorType,
        }),
    ...
    it("will render fallback ui when useFallbackUiForUnsupportedType is true", async () => {
      ...
      expect(
        node.querySelector<HTMLInputElement>("input[type=number]"),
      ).toBeInTheDocument();
      expect(
        node.querySelector<HTMLInputElement>("input[type=number]"),
      ).toHaveAttribute("value", "123456");
      ...
      let addButton = node.querySelector<HTMLButtonElement>(
        ".rjsf-object-property-expand button",
      );
      ...
      addButton = node.querySelector<HTMLButtonElement>(
  ...
  describe("Custom field template", () => {
    ...
    it("should use the provided template for labels", () => {
      expect(node.querySelector(".my-template > label")).toHaveTextContent(
        "root object",
      );
      expect(
        node.querySelector(".my-template .rjsf-field-string > label"),
    ...
    it("should pass help as a the provided React element", () => {
      expect(node.querySelector(".help-block")).toHaveTextContent(
        "this is help",
      );
    });

    it("should pass rawHelp as a string", () => {
      expect(node.querySelector(".raw-help")).toHaveTextContent(
  ...
  describe("ui options submitButtonOptions", () => {
    ...
    it("should render a submit button with text Confirm", () => {
      ...
      const { node } = createFormComponent(props);
      expect(node.querySelector("button[type=submit]")).toHaveTextContent(
        "Confirm",
      );
    });
  });
  ...
  describe("Schema definitions", () => {
    ...
    it("should raise for non-existent definitions referenced", () => {
      ...
      expect(() => createFormComponent({ schema })).toThrow(
    ...
    it("should recursively handle referenced definitions", () => {
      ...
      expect(
        node.querySelector("#root_children_0_name"),
  ...
  describe("Defaults additionalProperties propagation", () => {
    ...
    it("should submit a combination of properties and additional properties defaults", () => {
      ...
      expectToHaveBeenCalledWithFormData(
        onSubmit,
        { x: "x default value", y: "y default value" },
        true,
      );
    });
  ...
  describe("Change handler", () => {
    ...
    it("should call last provided change handler", async () => {
      ...
      const { onChange, rerender } = createFormComponent({
        ref: createRef(),
        schema,
        formData: { foo: "bar1" },
      });
    ...
    it("should modify an allOf field when the defaults are set", () => {
      ...
      expect(node.querySelector(secondInputID)).toHaveAttribute(
        "value",
        "second!",
      );
    ...
    it("should restore defaults when switching from null back to object option in oneOf", () => {
      ...
      const oneOfSelect = node.querySelector<HTMLSelectElement>(
        "#root__oneof_select",
      );
      ...
      expect(
        node.querySelector<HTMLSelectElement>("#root__oneof_select")!.value,
      ).toEqual("2");
      ...
      act(() => {
        fireEvent.change(
          node.querySelector<HTMLSelectElement>("#root__oneof_select")!,
          { target: { value: "1" } },
        );
      });
      ...
      const lastFormData =
        onChangeCalls[onChangeCalls.length - 1].event.formData;
  ...
  describe("External formData updates", () => {
    ...
    describe("array level", () => {
      it("should call submit handler with new formData prop value", async () => {
        ...
        const { node, onSubmit, rerender } = createFormComponent({
          ref: createRef(),
          schema,
        });
  ...
  describe("Error contextualization", () => {
    describe("on form state updated", () => {
      ...
      describe("Live validation", () => {
        ...
        it("should denote the new error in the field", () => {
          ...
          expect(node.querySelectorAll(".rjsf-field-error")).toHaveLength(1);
          expect(
            node.querySelector(".rjsf-field-string .error-detail"),
          ).toHaveTextContent("must NOT have fewer than 8 characters");
        });
      });
    ...
    describe("root level with multiple errors, live validation", () => {
      ...
      it("should denote the error in the field", () => {
        ...
        const liNodes = node.querySelectorAll(
          ".rjsf-field-string .error-detail li",
        );
    ...
    describe("nested field level, live validation", () => {
      ...
      it("should denote the error in the field", () => {
        ...
        const errorDetail = node.querySelector(
          ".rjsf-field-object .rjsf-field-string .error-detail",
        );
    ...
    describe("array indices, live validation", () => {
      ...
      it("should denote the error in the item field in error", () => {
        ...
        // live validate does not run on initial render anymore
        expect(fieldNodes[1].classList.contains("rjsf-field-error")).toBe(
          false,
        );
      ...
      it("should not denote errors on non impacted fields", () => {
        ...
        expect(fieldNodes[0].classList.contains("rjsf-field-error")).toBe(
          false,
        );
        expect(fieldNodes[2].classList.contains("rjsf-field-error")).toBe(
          false,
        );
      });
    });
    ...
    describe("nested array indices, live validation", () => {
      ...
      it("should denote the error in the nested item field in error", () => {
        ...
        let liNodes = node.querySelectorAll(
          ".rjsf-field-string .error-detail li",
        );
    ...
    describe("array nested items, live validation", () => {
      ...
      it("should denote the error in the array nested item", () => {
        ...
        const errors = [].map.call(liNodes, (li: Element) => li.textContent);
        expect(fieldNodes[1].classList.contains("rjsf-field-error")).toEqual(
          true,
        );
        expect(errors).toEqual(["must NOT have fewer than 4 characters"]);
      });
    ...
    describe("schema dependencies, live validation", () => {
      ...
      it("should only show error for property in selected branch", async () => {
        ...
        expect(onChange).toHaveBeenLastCalledWith(
          expect.objectContaining({
            errorSchema: {
              field1: { __errors: ["must have required property 'field1'"] },
            },
          }),
          "root_field1",
        );
      ...
      it("should only show errors for properties in selected branch", () => {
        ...
        act(() => {
          fireEvent.change(
            node.querySelectorAll<HTMLInputElement>("input[type=number]")[0],
            {
      ...
      it("customValidate should raise an error when End is larger than Start field.", () => {
        ...
        const customValidate = (formData: any, errors: FormValidation) => {
          if (formData["Start"] > formData["End"]) {
            errors["Start"]?.addError(
              "Validate error: Test should be LE than End",
            );
          }
          return errors;
        };
        ...
        let errorMessageContent = node.querySelector(
          "#root_Start__error .text-danger",
        );
        ...
        expect(node.querySelectorAll("#root_Start__error")).toHaveLength(1);
        errorMessageContent = node.querySelector(
          "#root_Start__error .text-danger",
        );
        expect(errorMessageContent).toHaveTextContent(
          "Validate error: Test should be LE than End",
  ...
  describe("Form disable prop", () => {
    ...
    it("should disable the submit button", () => {
      ...
      expect(
        node.querySelector("button[type='submit']:disabled"),
      ).toBeInTheDocument();
    });
  ...
  describe("Nested forms", () => {
    it("should call provided submit handler with form state", () => {
      ...
      const arraySubmit =
        arrayForm.querySelector<HTMLButtonElement>(".array-form-submit")!;
...
describe("Form omitExtraData and liveOmit", () => {
  it("should call omitExtraData when the omitExtraData prop is true and liveOmit is true", () => {
    ...
    const theSpy = jest
      .spyOn(ref.current!, "omitExtraData")
      .mockReturnValue({ foo: "" });
...
describe("omitExtraData on submit", () => {
  ...
  it("Should call validateFormWithFormData with the current formData if omitExtraData is false", () => {
    ...
    const { node } = createFormComponent(props);
    const theSpy = jest
      .spyOn(formRef.current!, "validateFormWithFormData")
...
describe("Async errors", () => {
  ...
  it("should reset when schema changes", () => {
    ...
    expect(formRef.current!.state.errorSchema).toEqual({
      foo: { __errors: ["must have required property 'foo'"] },
    });
    expect(formRef.current!.state.errors).toEqual([
      {
...
describe("Calling reset from ref object", () => {
  it("Reset API test", () => {
    ...
    expect(node.querySelector<HTMLInputElement>("input")).toHaveAttribute(
      "value",
      "",
    );
  });
...
describe("optionalDataControls", () => {
  ...
  it("does not render any optional data control messages when not turned on and readonly and disabled", () => {
    ...
    const { node } = createFormComponent(props);
    const addArrayControlNode = node.querySelector<HTMLButtonElement>(
      `#${arrayControlAddId}`,
    );
    const removeArrayControlNode = node.querySelector<HTMLButtonElement>(
      `#${arrayControlRemoveId}`,
    );
    const msgArrayControlNode = node.querySelector(`#${arrayControlMsgId}`);
    const addArrayBtn = node.querySelector(`#${arrayAddId}`);
    const addObjectControlNode = node.querySelector<HTMLButtonElement>(
      `#${objectControlAddId}`,
    );
    const removeObjectControlNode = node.querySelector<HTMLButtonElement>(
      `#${objectControlRemoveId}`,
    );
  ...
  it("only render object optional data controls when only object is turned on", () => {
    ...
    const addArrayBtn = node.querySelector(`#${arrayAddId}`);
    let addObjectControlNode = node.querySelector<HTMLButtonElement>(
      `#${objectControlAddId}`,
    );
    let removeObjectControlNode = node.querySelector<HTMLButtonElement>(
      `#${objectControlRemoveId}`,
    );
    ...
    addObjectControlNode = node.querySelector<HTMLButtonElement>(
      `#${objectControlAddId}`,
    );
    removeObjectControlNode = node.querySelector<HTMLButtonElement>(
      `#${objectControlRemoveId}`,
    );
  ...
  it("only render array optional data controls when only array is turned on", () => {
    ...
    const { node } = createFormComponent(props);
    let addArrayControlNode = node.querySelector<HTMLButtonElement>(
      `#${arrayControlAddId}`,
    );
    let removeArrayControlNode = node.querySelector<HTMLButtonElement>(
      `#${arrayControlRemoveId}`,
    );
    ...
    // now check to see if the UI adjusted
    addArrayControlNode = node.querySelector<HTMLButtonElement>(
      `#${arrayControlAddId}`,
    );
    removeArrayControlNode = node.querySelector<HTMLButtonElement>(
      `#${arrayControlRemoveId}`,
    );
...
describe("nameGenerator", () => {
  it("should generate bracket notation names for simple fields", () => {
    ...
    const { node } = createFormComponent({
      schema,
      nameGenerator: bracketNameGenerator,
    });
  ...
  it("should generate dot notation names for simple fields", () => {
    ...
    const { node } = createFormComponent({
      schema,
      nameGenerator: dotNotationNameGenerator,
    });
  ...
  it("should generate bracket notation names for nested objects", () => {
    ...
    expect(streetInput).toHaveAttribute(
      "name",
  ...
  it("should generate bracket notation names for array of objects", () => {
    ...
    expect(firstTaskTitleInput).toHaveAttribute(
      "name",
      "root[tasks][0][title]",
    );
    expect(firstTaskDoneInput).toHaveAttribute("name", "root[tasks][0][done]");
    expect(secondTaskTitleInput).toHaveAttribute(
      "name",
      "root[tasks][1][title]",
    );
