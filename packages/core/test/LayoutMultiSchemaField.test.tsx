...
describe("LayoutMultiSchemaField", () => {
  ...
  test("throws when no selectorField is provided", () => {
    ...
    expect(() => render(<LayoutMultiSchemaField {...props} />)).toThrow(
      expectedError,
    );
  ...
  test("default render with SIMPLE_ONEOF schema", async () => {
    ...
    const formControl =
      within(fakeFieldTemplate).getByTestId(SelectWidgetTestId);
    ...
    radios.forEach((radio, index) => {
      ...
      expect(radio.parentElement).toHaveTextContent(
        SIMPLE_ONEOF_OPTIONS[index].label,
      );
    });
  ...
  test("custom selector field, title and widget in uiSchema, formData, has error", async () => {
    ...
    expect(fakeFieldErrorTemplate).toHaveTextContent(
      get(props.errorSchema, [ERRORS_KEY])!.join(""),
    );
    ...
    const sanitizedFormData =
      props.registry.schemaUtils.sanitizeDataForNewSchema(
        retrievedOptions[0],
        retrievedOptions[1],
        props.formData,
      );
  ...
  describe("computeEnumOptions", () => {
    test("Reads oneOfs from refs", () => {
      ...
      const enumOptions = computeEnumOptions(
        schema,
        oneOfSchema[ONE_OF_KEY],
        schemaUtils,
        uiSchema,
      );
    ...
    test("throws error when no enumOptions are generated", () => {
      ...
      expect(() => computeEnumOptions({}, [], schemaUtils)).toThrow(
        "No enumOptions were computed from the schema {}",
      );
    });
  });
  describe("getSelectedOption", () => {
    ...
    test("no value, returns undefined", () => {
      expect(
        getSelectedOption(enumOptions, selectorField, undefined),
      ).toBeUndefined();
    });
    test("existing value,returns option with existing value", () => {
      expect(
        getSelectedOption(
          enumOptions,
          selectorField,
          SIMPLE_ONEOF_OPTIONS[0].value,
        ),
      ).toBe(SIMPLE_ONEOF[ONE_OF_KEY]![0]);
    });
    test("non-existing value, returns undefined", () => {
      expect(
        getSelectedOption(enumOptions, selectorField, "randomValue"),
      ).toBeUndefined();
    });
  });
});
