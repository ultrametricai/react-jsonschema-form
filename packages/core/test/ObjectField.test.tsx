...
describe("ObjectField", () => {
  describe("schema", () => {
    ...
    it("should render a default property label", () => {
      ...
      expect(node.querySelector(".rjsf-field-boolean label")).toHaveTextContent(
        "bar",
      );
    });
    ...
    it("should render a string property", () => {
      ...
      expect(
        node.querySelectorAll(".rjsf-field input[type=text]"),
      ).toHaveLength(1);
    });
    ...
    it("should render a boolean property", () => {
      ...
      expect(
        node.querySelectorAll(".rjsf-field input[type=checkbox]"),
      ).toHaveLength(1);
    });
    ...
    it("should handle a default object value", () => {
      ...
      expect(node.querySelector(".rjsf-field input[type=text]")).toHaveValue(
        "hey",
      );
      expect(
        node.querySelector(".rjsf-field input[type=checkbox]"),
      ).toBeChecked();
    });
    ...
    it("should handle required values", () => {
      ...
      expect(node.querySelector("input[type=text]")).toHaveAttribute(
        "required",
        "",
      );
      expect(node.querySelector(".rjsf-field-string label")).toHaveTextContent(
        "Foo*",
      );
    });
    ...
    it("should render the widget with the expected id", () => {
      ...
      expect(node.querySelector("input[type=checkbox]")).toHaveAttribute(
        "id",
        "root_bar",
      );
    });
    ...
    it("Check that when formData changes, the form should re-validate", () => {
      ...
      expect(errorMessages).toHaveLength(1);
      const errorMessageContent = node.querySelector(
        "#root_foo__error .text-danger",
      );
  ...
  describe("fields ordering", () => {
    ...
    it("should use provided order", () => {
      ...
      const labels = [].map.call(
        node.querySelectorAll(".rjsf-field > label"),
        (l: Element) => l.textContent,
      );
    ...
    it("should throw when order list misses an existing property", () => {
      ...
      expect(node.querySelector(".rjsf-config-error")).toHaveTextContent(
  ...
  describe("additionalProperties", () => {
    ...
    it("should render a label for the additional property key", () => {
      ...
      expect(node.querySelector("[for='root_first-key']")).toHaveTextContent(
        "first Key",
      );
    });
    ...
    it("should render a label for the additional property value", () => {
      ...
      expect(node.querySelector("[for='root_first']")).toHaveTextContent(
        "first",
      );
    });
    ...
    it("should have an expand button", () => {
      ...
      expect(
        node.querySelector(".rjsf-object-property-expand button"),
      ).not.toBeNull();
    });
    ...
    it("should edit the specified default key without duplicating", () => {
      ...
      fireEvent.blur(node.querySelector("#root_defaultKey-key")!, {
        target: { value: "newDefaultKey" },
      });
  ...
  describe("markdown", () => {
    ...
    it("should render markdown in description when enableMarkdownInDescription is set to true", () => {
      ...
      const field = node.querySelector(
        "form .form-group .form-group .field-description",
      );
      expect(field).toContainHTML(
        "New <em>description</em>, with some Markdown.",
      );

      const details = node.querySelector(
        "form .form-group .form-group .rjsf-field-string .field-description",
      );
      expect(details).toContainHTML(
        "Description to renders Markdown <strong>correctly</strong>.",
      );
      ...
      expect(checkbox).toContainHTML(
        "Checkbox with some <code>markdown</code>!",
      );
    });
    it("should not render markdown in description when enableMarkdownInDescription is not present in uiSchema", () => {
      ...
      expect(details).toContainHTML(
        "Description to renders Markdown **correctly**.",
      );
