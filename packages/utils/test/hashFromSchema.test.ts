...
describe("sortedJSONStringify", () => {
  ...
  it("stringifies the object putting keys in order", () => {
    expect(sortedJSONStringify(OUT_OF_ORDER_SCHEMA)).toEqual(
      STRINGIFIED_IN_ORDER,
    );
  });
});
...
const TEST_OBJECT_1 = {
  ...
  yes: "no",
  bool: false,
  nested: { one: "1", two: 2, three: 3.0 },
};
const TEST_OBJECT_2 = {
  bool: false,
  foo: "bar",
  nested: { one: "1", three: 3.0, two: 2 },
  yes: "no",
};
...
describe("hashObject", () => {
  ...
  describe("handles arrays", () => {
    ...
    beforeAll(() => {
      ...
      expected3 = hashString(
        JSON.stringify([TEST_ARRAY_3[0], [TEST_ARRAY_2[0], TEST_OBJECT_2]]),
      );
    });
