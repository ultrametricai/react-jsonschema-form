...
export default function findFieldInSchemaTest(
  ...
  describe("findFieldInSchema", () => {
    ...
    it("return NOT_FOUND when field does not exist in the schema", () => {
      expect(schemaUtils.findFieldInSchema(simpleSchema, "foo")).toEqual(
        NOT_FOUND,
      );
    });
    ...
    it("schema has oneOf field in properties key and isRequired true", () => {
      const path = "answer";
      expect(
        schemaUtils.findFieldInSchema(testOneOfSchema, path, ANSWER_1),
      ).toEqual({
    ...
    it("schema has anyOf field in properties key and isRequired false", () => {
      const path = "answer";
      expect(
        schemaUtils.findFieldInSchema(testAnyOfSchema, path, ANSWER_1),
      ).toEqual({
        field: expectedAnswerField,
        isRequired: true,
      });
    ...
    it("schema has oneOf in nested field in properties key and isRequired true", () => {
      ...
      expect(
        schemaUtils.findFieldInSchema(nestedOneOf, path, { nested: ANSWER_1 }),
      ).toEqual({
        field: expectedAnswerField,
        isRequired: true,
      });
    ...
    it("schema has anyOf in nested field in properties key and isRequired false", () => {
      const path = "nested.answer";
      expect(
        schemaUtils.findFieldInSchema(nestedAnyOf, path, { nested: ANSWER_1 }),
      ).toEqual({
        field: expectedAnswerField,
        isRequired: true,
      });
