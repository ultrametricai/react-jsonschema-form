...
export default function findSelectedOptionInXxxOfTest(
  ...
  describe("findSelectedOptionInXxxOf", () => {
    test("returns undefined when schema has no oneOfs", () => {
      expect(
        schemaUtils.findSelectedOptionInXxxOf({}, "foo", ONE_OF_KEY),
      ).toBeUndefined();
    });
    test("returns undefined when schema has no anyOfs", () => {
      expect(
        schemaUtils.findSelectedOptionInXxxOf({}, "foo", ANY_OF_KEY),
      ).toBeUndefined();
    });
