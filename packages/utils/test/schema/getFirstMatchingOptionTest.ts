...
export default function getFirstMatchingOptionTest(
  ...
  describe("getFirstMatchingOption()", () => {
    ...
    it("should return 0 when schema has discriminator but no matching data", () => {
      ...
      expect(
        getFirstMatchingOption(testValidator, null, options, schema, "code"),
      ).toEqual(0);
    });
    ...
    it("should return Bar when schema has simple discriminator for bar", () => {
      ...
      expect(
        schemaUtils.getFirstMatchingOption(formData, options, "code"),
      ).toEqual(1);
    });
    ...
    it("should return Bar when schema has non-simple discriminator for bar", () => {
          ...
          Foo: {
            ...
            properties: {
              code: {
                title: "Code",
                type: "array",
                items: { type: "string", enum: ["foo_coding"] },
              },
            },
          },
          Bar: {
            ...
            properties: {
              code: {
                title: "Code",
                type: "array",
                items: { type: "string", enum: ["bar_coding"] },
              },
            },
          },
        },
      ...
      // Use the schemaUtils to verify the discriminator prop gets passed
      const schemaUtils = createSchemaUtils(testValidator, schema);
      const result = schemaUtils.getFirstMatchingOption(
        formData,
        options,
        "code",
      );
      ...
      if (wasWarned) {
        ...
        expect(consoleWarnSpy).toHaveBeenCalledWith(
          ...
          expect.objectContaining({
            message:
              'discriminator: "properties/code" must have "const" or "enum"',
          }),
        );
