...
export default function getDisplayLabelTest(testValidator: TestValidatorType) {
  describe("getDisplayLabel()", () => {
    it('with global uiSchema "label" set to true', () => {
      expect(
        getDisplayLabel(testValidator, { type: "string" }, {}, undefined, {
          label: true,
        }),
      ).toEqual(true);
    });
    it('with global uiSchema "label" set to false', () => {
      ...
      expect(schemaUtils.getDisplayLabel(schema, {}, { label: false })).toEqual(
        false,
      );
    });
    ...
    describe("array type", () => {
      it("added by additionalProperty", () => {
        expect(
          getDisplayLabel(
            testValidator,
            { type: "array", [ADDITIONAL_PROPERTY_FLAG]: true },
            {},
          ),
        ).toEqual(true);
      });
