...
describe("getWidget()", () => {
  it("should fail if widget has incorrect type", () => {
    expect(() => getWidget(schema)).toThrow(
      `Unsupported widget definition: undefined in schema: ${schemaStr}`,
    );
  });

  it("should fail if widget has no type property", () => {
    expect(() => getWidget(schema, "blabla")).toThrow(
      `No widget for type 'object' in schema: ${schemaStr}`,
    );
  });
  ...
  it("should fail if schema has no type property", () => {
    expect(() => getWidget({}, "blabla")).toThrow(
      `No widget 'blabla' for type 'undefined' in schema: {}`,
    );
  });
