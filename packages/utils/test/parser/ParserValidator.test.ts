...
describe("ParserValidator", () => {
  ...
  it("calling isValid() with TINY_SCHEMA returns false", () => {
    expect(validator.isValid(TINY_SCHEMA, undefined, RECURSIVE_REF)).toBe(
      false,
    );
  });
  ...
  it("calling isValid() with a schema that has a matching key throws error", () => {
    ...
    expect(() =>
      validator.isValid(DUPLICATE_SCHEMA, undefined, RECURSIVE_REF),
    ).toThrow(
      new Error(
  ...
  it("the first shows the existing schema", () => {
    expect(consoleErrorSpy).toHaveBeenNthCalledWith(
      1,
      "existing schema:",
      JSON.stringify(TINY_SCHEMA, null, 2),
    );
  });
  it("the second shows the new schema", () => {
    expect(consoleErrorSpy).toHaveBeenNthCalledWith(
      ...
      "new schema:",
      JSON.stringify(
        { ...DUPLICATE_SCHEMA, [ID_KEY]: DUPLICATE_HASH },
        null,
        2,
      ),
    );
