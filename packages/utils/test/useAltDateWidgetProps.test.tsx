...
function DateElementsTester(
  props: WidgetProps & {
    ...
    handleChange: UseAltDateWidgetResult["handleChange"];
  },
) {
  const {
    elements,
...
describe("useAltDateWidgetProps()", () => {
  ...
  test("time is false, value undefined", () => {
    ...
    const expectedElements = getDateElementProps(
      dateObj,
      false,
      PROPS.options.yearsRange,
    );
  ...
  test("time is false, value undefined, testing DateElements", () => {
    ...
    const { elements, handleChange, handleClear, handleSetNow } =
      result.current;
    const dateObj = parseDateString();
    let expectedElements = getDateElementProps(
      dateObj,
      false,
      PROPS.options.yearsRange,
    );
    ...
    dateObj.year = 2023;
    expectedElements = getDateElementProps(
      dateObj,
      false,
      PROPS.options.yearsRange,
    );
  ...
  test("time is true, value undefined, testing DateElements", () => {
    const { result, rerender: rerenderHook } = renderHook(() =>
      useAltDateWidgetProps(TIME_PROPS),
    );
    const { elements, handleChange, handleClear, handleSetNow } =
      result.current;
    const dateObj = parseDateString();
    ...
    expect(TIME_PROPS.onChange).toHaveBeenCalledWith(
      "2023-10-02T01:02:03.000Z",
    );
  });
