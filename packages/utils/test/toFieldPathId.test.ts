...
describe("toFieldPathId()", () => {
  test("fieldPath is empty string", () => {
    expect(toFieldPathId("", GLOBAL_FORM_OPTIONS)).toEqual({
      path: [],
  ...
  describe("with nameGenerator", () => {
    ...
    test("generates name for string fieldPath", () => {
      ...
      const result = toFieldPathId(
        path,
        GLOBAL_FORM_OPTIONS_WITH_NAME_GENERATOR,
      );
    ...
    test("generates name with isMultiValue flag for multi-select fields", () => {
      const phpMultiValueNameGenerator = (
        path: (string | number)[],
        idPrefix: string,
      ...
      const result = toFieldPathId(
        "",
        GLOBAL_FORM_OPTIONS_WITH_MULTI_VALUE,
        parentPath,
        true,
      );
      expect(result).toEqual({
