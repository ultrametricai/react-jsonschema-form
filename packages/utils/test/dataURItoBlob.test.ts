...
describe("dataURItoBlob()", () => {
  ...
  it("should return the body is empty", () => {
    const { blob, name } = dataURItoBlob(
      "data:application/json;name=test.png;base64,",
    );
  ...
  it("should throw when the body is not a Base64 encoded dataURI", () => {
    expect(() => dataURItoBlob("data:;Hello%20World")).toThrow(
  ...
  it("should throw if the body is not a valid dataURI", () => {
    expect(() => dataURItoBlob("Hello%20World")).toThrow(
      new Error("File is invalid: URI must be a dataURI"),
    );
    expect(() => dataURItoBlob("javascript:alert()")).toThrow(
      new Error("File is invalid: URI must be a dataURI"),
    );
  });
  ...
  it("should throw the body is not valid Base64", () => {
    expect(() => dataURItoBlob("data:text/plain;base64,Hello%20World")).toThrow(
      new Error(
        "File is invalid: The string to be decoded contains invalid characters.",
      ),
    );
