...
describe("getChangedFields()", () => {
  it("Empty parameter", () => {
    ...
    expect(getChangedFields(complexObject, undefined)).toEqual(
      complexObjectKeys,
    );
    expect(getChangedFields(undefined, complexObject)).toEqual(
      complexObjectKeys,
    );
  });
  ...
  it("One is not plainObject parameter", () => {
    ...
    expect(getChangedFields("1", complexObject)).toEqual(complexObjectKeys);
    expect(getChangedFields(function noop() {}, complexObject)).toEqual(
      complexObjectKeys,
    );
    expect(getChangedFields(new Date(), complexObject)).toEqual(
      complexObjectKeys,
    );

    expect(getChangedFields(complexObject, 1)).toEqual(complexObjectKeys);
    expect(getChangedFields(complexObject, "1")).toEqual(complexObjectKeys);
    expect(getChangedFields(complexObject, function noop() {})).toEqual(
      complexObjectKeys,
    );
    expect(getChangedFields(complexObject, new Date())).toEqual(
      complexObjectKeys,
    );
  });
  it("Deep equal", () => {
  ...
  it("Change one field", () => {
    expect(
      getChangedFields(complexObject, { ...cloneDeep(complexObject), a: 2 }),
    ).toEqual(["a"]);
    expect(
      getChangedFields({ ...cloneDeep(complexObject), a: 2 }, complexObject),
    ).toEqual(["a"]);
  });
