...
describe("findSchemaDefinition()", () => {
  it("throws error when ref is missing", () => {
    expect(() => findSchemaDefinition()).toThrow(
  ...
  it("throws error when bundled ref are not part of JSON Schema Draft 2020-12", () => {
    expect(() =>
      findSchemaDefinition("#/definitions/bundledRef", schema),
    ).toThrow("Could not find a definition for /bundled.ref.json");
  });
  ...
  it("correctly resolves absolute bundled refs in arrays within a JSON Schema Draft 2020-12", () => {
    expect(
      findSchemaDefinition(
        "#/$defs/bundledAbsoluteRefWithinArray",
        bundledSchema,
      ),
    ).toBe((bundledSchema.$defs!.bundledSchemaArray as RJSFSchema).anyOf![0]);
  });
  ...
  it("correctly resolves refs with explicit base URI in a bundled JSON Schema", () => {
    expect(
      findSchemaDefinition(
        "bundled.ref.json",
        bundledSchema,
        "https://example.com/undefined.ref.json",
      ),
    ).toStrictEqual(internalSchema);
  });
  ...
  it("throws error when relative ref is undefined in a bundled JSON Schema", () => {
    expect(() =>
      findSchemaDefinition("#/$defs/undefinedRef", bundledSchema),
    ).toThrow("Could not find a definition for /undefined.ref.json");
  });
  it("throws error when relative ref with anchor is undefined in a bundled JSON Schema", () => {
    expect(() =>
      findSchemaDefinition("#/$defs/undefinedRefWithAnchor", bundledSchema),
    ).toThrow("Could not find a definition for #/$defs/undefined");
  });
  ...
  it("throws error when ref is a deep circular reference in a bundled JSON Schema", () => {
    expect(() =>
      findSchemaDefinition("#/$defs/circularRef", bundledSchema),
    ).toThrow(
...
describe("findSchemaDefinitionRecursive()", () => {
  it("throws error when ref is missing", () => {
    expect(() => findSchemaDefinitionRecursive()).toThrow(
      "Could not find a definition for undefined",
    );
  });
  it("throws error when ref is malformed", () => {
  ...
  it("throws error when ref does not exist", () => {
    expect(() =>
      findSchemaDefinitionRecursive("#/definitions/missing", schema),
    ).toThrow("Could not find a definition for #/definitions/missing");
  });
  it("returns the string ref from its definition", () => {
    expect(
      findSchemaDefinitionRecursive("#/definitions/stringRef", schema),
    ).toBe(schema.definitions!.stringRef);
  });
  it("returns the string ref from its nested definition", () => {
    expect(
      findSchemaDefinitionRecursive("#/definitions/nestedRef", schema),
    ).toBe(schema.definitions!.stringRef);
  });
  it("returns a combined schema made from its nested definition with the extra props", () => {
    expect(
      findSchemaDefinitionRecursive("#/definitions/extraNestedRef", schema),
    ).toEqual(EXTRA_EXPECTED);
  });
  it("throws error when ref is a circular reference", () => {
    expect(() =>
  ...
  it("throws error when bundled ref are not part of JSON Schema Draft 2020-12", () => {
    expect(() =>
      findSchemaDefinitionRecursive("#/definitions/bundledRef", schema),
    ).toThrow("Could not find a definition for /bundled.ref.json");
  });
  it("throws error when bundled ref with explicit baseURI are not part of JSON Schema Draft 2020-12", () => {
    expect(() =>
  ...
  it("correctly resolves indirect bundled refs within a JSON Schema Draft 2020-12", () => {
    expect(
      findSchemaDefinitionRecursive("#/$defs/indirectRef", bundledSchema),
    ).toStrictEqual(internalSchema);
  });
  ...
  it("throws error when relative ref is undefined in a bundled JSON Schema", () => {
    expect(() =>
      findSchemaDefinitionRecursive("#/$defs/undefinedRef", bundledSchema),
    ).toThrow("Could not find a definition for /undefined.ref.json");
  });
  it("throws error when relative ref with anchor is undefined in a bundled JSON Schema", () => {
    expect(() =>
      findSchemaDefinitionRecursive(
        "#/$defs/undefinedRefWithAnchor",
        bundledSchema,
      ),
    ).toThrow("Could not find a definition for #/$defs/undefined");
  });
  ...
  it("throws error when ref is a deep circular reference in a bundled JSON Schema", () => {
    expect(() =>
      findSchemaDefinitionRecursive("#/$defs/circularRef", bundledSchema, []),
    ).toThrow(
      "Definition for #/$defs/circularRef contains a circular reference through /bundled.ref.json/#/$defs/circularRef -> /bundled.schema.json/#/$defs/circularRef -> #/$defs/circularRef",
    );
  });
...
describe("makeAllReferencesAbsolute()", () => {
  it("correctly makes all references absolute in a JSON Schema", () => {
    expect(
      makeAllReferencesAbsolute(internalSchema, internalSchema[ID_KEY]!),
    ).toStrictEqual({
      ...internalSchema,
      $defs: {
        ...internalSchema.$defs,
        circularRef: {
          $ref: "https://example.com/bundled.schema.json/#/$defs/circularRef",
        },
        undefinedRef: {
          $ref: "https://example.com/bundled.ref.json#/$defs/undefined",
        },
      },
      properties: {
