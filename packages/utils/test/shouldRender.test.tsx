...
describe("shouldRender()", () => {
  describe("default strategy (customDeep)", () => {
    describe("single level comparison checks", () => {
      ...
      it("should detect equivalent props and state", () => {
        expect(shouldRender(initial, { myProp: 1 }, { myState: 1 })).toBe(
          false,
        );
      });
      ...
      it("should handle equivalent function prop", () => {
        ...
        expect(shouldRender(initial, { myProp: fn }, { myState: 1 })).toBe(
          false,
        );
      });
    });
  ...
  describe("always strategy (explicit)", () => {
    ...
    it("should return true even with identical props and state", () => {
      expect(
        shouldRender(initial, initial.props, initial.state, "always"),
      ).toBe(true);
    });

    it("should return true with different props", () => {
      expect(
        shouldRender(initial, { myProp: 2 }, { myState: 1 }, "always"),
      ).toBe(true);
    });

    it("should return true with different state", () => {
      expect(
        shouldRender(initial, { myProp: 1 }, { myState: 2 }, "always"),
      ).toBe(true);
    });
  });
  ...
  describe("shallow strategy", () => {
    ...
    it("should use shallow comparison for props and state", () => {
      ...
      expect(
        shouldRender(initial, { myProp: 1 }, { myState: 1 }, "shallow"),
      ).toBe(false);

      // Should re-render when props differ
      expect(
        shouldRender(initial, { myProp: 2 }, { myState: 1 }, "shallow"),
      ).toBe(true);

      // Should re-render when state differs
      expect(
        shouldRender(initial, { myProp: 1 }, { myState: 2 }, "shallow"),
      ).toBe(true);
    });
    ...
    it("should re-render for nested object changes (shallow comparison)", () => {
      ...
      expect(
        shouldRender(initial, { myProp: obj2 }, { myState: 1 }, "shallow"),
      ).toBe(true);
    });
  });
  ...
  describe("edge cases and error handling", () => {
    ...
    it("should handle invalid strategy gracefully", () => {
      ...
      expect(
        shouldRender(initial, { myProp: 1 }, { myState: 1 }, "invalid" as any),
      ).toBe(false); // Should fall through to customDeep
    });
  });
});
