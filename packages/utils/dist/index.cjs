...
function makeAllReferencesAbsolute(schema, baseURI) {
  ...
  if (REF_KEY in schema) {
    schema = {
      ...schema,
      [REF_KEY]: import_fast_uri.default.resolve(currentURI, schema[REF_KEY])
    };
  }
  for (const [key, subSchema] of Object.entries(schema)) {
    if (Array.isArray(subSchema)) {
      schema = {
        ...schema,
        [key]: subSchema.map(
          (item) => (0, import_isObject3.default)(item) ? makeAllReferencesAbsolute(item, currentURI) : item
        )
      };
    } else if ((0, import_isObject3.default)(subSchema)) {
      schema = {
  ...
  if (ref.startsWith("#")) {
    ...
    if (current !== void 0) {
      ...
      if (!(0, import_isEmpty.default)(refAnchor)) {
        current = import_jsonpointer.default.get(
          current,
...
function getOptionMatchingSimpleDiscriminator(formData, options, discriminatorField) {
  if (formData && discriminatorField) {
    ...
    for (let i = 0; i < options.length; i++) {
      ...
      const discriminator = (0, import_get3.default)(
        option,
...
function getFirstMatchingOption(validator, formData, options, rootSchema, discriminatorField) {
  ...
  for (let i = 0; i < options.length; i++) {
    ...
    if (discriminatorField && (0, import_has.default)(option, [PROPERTIES_KEY, discriminatorField])) {
      ...
      const discriminator = (0, import_get4.default)(
        option,
        [PROPERTIES_KEY, discriminatorField],
        {}
      );
...
function getAllPermutationsOfXxxOf(listOfLists) {
  const allPermutations = listOfLists.reduce(
    (permutations, list) => {
      if (list.length > 1) {
        return list.flatMap(
          (element) => (0, import_times.default)(
            permutations.length,
...
var shallowAllOfMerge = (0, import_json_schema_merge.createShallowAllOfMerge)(
    ...
    if (data !== void 0) {
      return xxxOfs.find((xxx2) => {
        return (0, import_isEqual.default)(
          (0, import_get6.default)(
            xxx2,
            [PROPERTIES_KEY, selectorField, DEFAULT_KEY],
            (0, import_get6.default)(xxx2, [PROPERTIES_KEY, selectorField, CONST_KEY])
          ),
          data
        );
      });
...
function optionsList(schema, uiSchema) {
  ...
  return altSchemas && altSchemas.map((aSchemaDef, index) => {
    ...
    let label = title;
    if (selectorField) {
      const innerSchema = (0, import_get11.default)(
        aSchema,
...
function getObjectDefaults(validator, rawSchema, {
...
} = {}, defaults) {
  {
    ...
    const objectDefaults = Object.keys(retrievedSchema.properties || {}).reduce(
      (acc, key) => {
        const propertySchema = (0, import_get12.default)(
          retrievedSchema,
          [PROPERTIES_KEY, key],
          {}
        );
...
function getDisplayLabel(validator, schema, uiSchema = {}, rootSchema, globalOptions, experimental_customMergeAllOf) {
  ...
  if (displayLabel) {
    const schemaType = getSchemaType(schema);
    const addedByAdditionalProperty = (0, import_get13.default)(
      schema,
  ...
  constructor(validator, rootSchema, experimental_defaultFormStateBehavior, experimental_customMergeAllOf) {
    if (rootSchema && rootSchema[SCHEMA_KEY] === JSON_SCHEMA_DRAFT_2020_12) {
      this.rootSchema = makeAllReferencesAbsolute(
        rootSchema,
        (0, import_get17.default)(rootSchema, ID_KEY, "#")
      );
    } else {
...
var ErrorSchemaBuilder = class {
  ...
  getOrCreateErrorBlock(pathOfError) {
    ...
    let errorBlock = hasPath ? (
      ...
      (0, import_get18.default)(this.errorSchema, pathOfError)
    ) : this.errorSchema;
...
function getChangedFields(a, b) {
  ...
  if (aIsPlainObject && !bIsPlainObject) {
    ...
    const unequalFields = (0, import_keys.default)(
      (0, import_pickBy.default)(a, (value, key) => !deepEquals(value, (0, import_get19.default)(b, key)))
    );
